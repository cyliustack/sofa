...
if __name__ == '__main__':
    ...
    if args.recover:     
        ...
        ret = call(sudo + 'chgrp root ' + ' ' + args.utility, shell=True)
        if args.remove_group_sofa:
            print('The group "sofa" is going to be removed. Are you sure (y/n)?', end=' ')
            if input() == 'y':
                call(sudo + 'groupdel sofa', shell=True)
        sys.exit(0)
    else:
        call(sudo + 'groupadd sofa', shell=True)
        ret = call(sudo + 'chgrp sofa ' + args.utility, shell=True)
        if ret != 0:
            print('Failed...')
            sys.exit(1)
        call(sudo + 'chmod g+rx ' + args.utility, shell=True)

    #Set capabilities for the specified utility
    if args.utility.find('tcpdump') != -1:
        ret = call(sudo + 'setcap cap_net_raw,cap_net_admin=eip ' + args.utility, shell=True)
    ...
    if ret != 0:
        ...
        inform_sudo()
        call(sudo + 'usermod -a -G sofa %s' % args.user, shell=True)
    print('Every thing is ready.')

    #call(sudo + 'newgrp sofa', shell=True) 
